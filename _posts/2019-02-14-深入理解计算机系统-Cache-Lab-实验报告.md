...
void cache_organized(int set_index_bits, int lines_per_set, int block_bits);
void process_trace_file_line(char *line_pointer, int *hit_count, int *miss_count, int *eviction_count);
void read_trace_file(char *trace_path, int *hit_count, int *miss_count, int *eviction_count);

void cache_organized(cache_sim *cache, int set_index_bits, int lines_per_set){
	cache = (cache_sim *)malloc(sizeof(cache_line_sim))
}
...
// S 28,4
// S 50,4
void process_trace_file(char *trace_path, int *hit_count, int *miss_count, int *eviction_count){
    //  以只读模式打开文件
	...
	while( fgets(line_buff, 255, fp) != EOF ){ 
		...
		line_pointer++;
		process_trace_file_line(line_pointer, hit_count, miss_count, eviction_count);
	}
}
...
int main(int argc, char *argv[]){
	...
	cache_organized(set_index_bits, lines_per_set, block_bits);

	process_trace_file(trace_path, hit_count, miss_count, eviction_count)
