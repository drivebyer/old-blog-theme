...
void process_trace_file_line(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count);
void process_trace_file(cache_sim *cache, char *trace_path, int *hit_count, int *miss_count, int *eviction_count);
void data_load_operation(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count);
void data_store_operation(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count);
void data_modify_operation(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count);
// 读取该文件中的内容
// L 10,4
// S 18,4
// L 20,4
...
void process_trace_file(cache_sim *cache, char *trace_path, int *hit_count, int *miss_count, int *eviction_count){
	...
	while( fgets(line_buff, 255, fp) != EOF ){ 
		...
		line_pointer++;
		process_trace_file_line(cache, line_pointer, hit_count, miss_count, eviction_count);
	}
}
...
void process_trace_file_line(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count){
	...
	switch(line_pointer){
		case 'L':
			data_load_operation(cache, line_pointer, hit_count, miss_count, eviction_count);
		break;
		case 'S':
			data_store_operation(cache, line_pointer, hit_count, miss_count, eviction_count);
		break;
		case 'M':
			data_modify_operation(cache, line_pointer, hit_count, miss_count, eviction_count);
		break;
		default:
		break;
	}
...
void data_load_operation(cache_sim *cache, char *line_pointer, int *hit_count, int *miss_count, int *eviction_count){
	line_pointer++;
	// 现在 line_pointer 指向第二个字节
	int address;
	char *address_str;
	char *comma;
	comma = strchr(line_pointer,',');
	strncpy(address_str, line_pointer, comma - line_pointer);
...
int main(int argc, char *argv[]){
	...
	while ((opt = getopt(argc, argv, "vs:E:b:t:")) != -1) {
		switch (opt) {
			...
			break;
			case 's':
			set_index_bits_number = atoi(optarg);
			break;
			case 'E':
	...
	cache = (cache_sim *)malloc(sizeof(cache_line_sim) * lines_per_set * pow(2, set_index_bits_number));

	process_trace_file(cache, trace_file_path, hit_count, miss_count, eviction_count);
