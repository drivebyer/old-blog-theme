...
世界上第一台 PC，是基于 16 位都 Intel 8088 处理器，但是那时的地址线有 20 bit，所以能寻址到 2^20 byte(1MB)，所以早期 PC 的内存地址范围为 0x00000000 ~ 0x000FFFFF(640KB)，图中的 Low Memory 是早期 PC 能够使用的 RAM 范围。
...
这两部分被 0x000A0000 ~ 0x00100000 这块区域分割开来。另外，在 32 位 PC 架构的机器上，高地址处通常为 PCI 设备保留着。
