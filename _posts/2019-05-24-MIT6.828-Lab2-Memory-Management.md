...
回想一下 lab1，我们在 entrypgdir.c 设置了一个简陋的页目录和页表：
...
通过 Page Table 里最后一个 PTE：0x3ff000|PTE_P|PTE_W，也可以看出映射到的物理地址最多也不会超过 0x003ff000+0xFFF。

上面的 Page Directory，Page Table，GDT 都被编译进 kernel 里了，所以在加载完 kernel 后，这 3 张表的内容自然就存放在了物理内存 kernel 的范围里。

一旦启动保护模式后，任何使用到的地址都是虚拟地址（VA），即使是 kernel 也不例外，但由于 kernel 是最接近底层硬件的软件，所以有时候还是会使用到物理内存，例如在建立 PDE 和 PTE 的时候，存放在其中的 PPN 就是物理地址，为了在 JOS kernel 中加以区分：**uintptr_t** 表示虚拟地址，**physaddr_t** 表示物理地址，它们都是定义自 **uint32_t**。

如果我们在内核中遵守了这个约定，就不能对 **physaddr_t** 解引用，因为凡是我们在软件（包括kernel）中使用到的地址，MMU 都会将其当做 VA，即使我们在编程时将它当做物理地址（physaddr_t），但是 MMU 还是会将其当做 VA，并将其翻译成 PA，这样一来就会解析到错误的内存地址处。
...
因为在第 3 行对 value 进行了解引用，说明 value 指针里是一个虚拟地址，那么在第 4 行里只能将这个指针转换成 uintptr_t 类型。

```
31       24  23
+---------+-+-+-+---+----------+-+---+-+----+---------+
|Base31:24|G|0|0|ALV|limit19:16|P|DPL|0|type|base23:16|
+---------+-+-+-+---+----------+-+---+-+----+---------+
