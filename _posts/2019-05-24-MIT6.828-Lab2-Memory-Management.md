...
在前面学到，启动保护模式后，任何使用到的地址都是虚拟地址（VA），即使是 kernel 也不例外，但由于 kernel 是最接近底层硬件的软件，所以有时候还是会使用到物理内存，例如在建立 PDE 和 PTE 的时候，存放在其中的 PPN 就是物理地址，为了在 JOS kernel 中加以区分：**uintptr_t** 表示虚拟地址，**physaddr_t** 表示物理地址，它们都是定义自 **uint32_t**。
