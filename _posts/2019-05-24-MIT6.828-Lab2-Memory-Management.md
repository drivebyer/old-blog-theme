...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  for(;;){
    if((pte = pgdir_walk(pgdir, a, 1)) == 0) /*walkpgdir()中已经设置好PDE*/
      return -1;
    if(*pte & PTE_P)
      panic("remap");
    /*
...
为了计算方便，我将 xv6 中的两个宏添加到了 JOS 的 **mmu.h** 中：
...
**TODO**：可是我有个疑问，既然只使用到了 [KSTACKTOP-KSTKSIZE, KSTACKTOP) 部分空间，为什么还要区分 CPU0 与 CPU1 的内核栈呢？
...
PTE 和 PDE 分为高 20bits 和低 12bits，其中高 20bits 指向一个物理页，低 12bits 用来描述当前指令对这个物理页的操作许可。
